...
func TestGetSanrioNewsReleaseFromDocument(t *testing.T) {
	...
	assert.Equal(t, "/wp-content/uploads/2015/05/20160708-1.pdf", feed.Items[0].Link.Href)
	assert.Equal(t, time.Date(2016, 7, 8, 0, 0, 0, 0, loc), feed.Items[0].Created)
	assert.Equal(t, "2016年バレンタイン向けスペシャルギフト「GODIVA &ハローキティ」・「GODIVA &マイメロディ」1月6日（水）よりサンリオ限定販売", feed.Items[50].Title)
	assert.Equal(t, "/corporate/release/y2016/d0106/", feed.Items[50].Link.Href)
	assert.Equal(t, time.Date(2016, 1, 6, 0, 0, 0, 0, loc), feed.Items[50].Created)
}
